...
export class UpdateProfileCommand implements ICommand<Nullable<URL>> {
  constructor(
    public readonly userId: UserId,
    public readonly firstName?: Nullable<FirstName>,
    public readonly lastName?: Nullable<LastName>,
    public readonly avatarId?: Nullable<NonEmptyString>,
  ) {}
}
...
> {
  ...
  handle(command: UpdateProfileCommand): ResultAsync<Nullable<URL>> {
    return this.userRepository.getById(command.userId).andThen((user) => {
      ...
      const isUpdated = user.update(command.firstName, command.lastName, command.avatarId);
      if (!isUpdated) return okAsync(null);
      ...
      return this.userRepository
        .update(user)
        .andThen(() =>
          user.avatarId ? this.objectSTorage.createDownloadUrl(user.avatarId) : okAsync(null),
        );
    });
  }
}
