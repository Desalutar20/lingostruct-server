...
export class OutboxUserBannedProcessor extends CronProcessor {
  constructor(
  ...
  protected async execute(): Promise<void> {
    await this.unitOfWork.execute(({ outboxRepository }, { commit, rollback }) =>
      outboxRepository
        .getAndLockPendingOutboxes<OutboxUserBannedData>(
          OutboxType.UserBanStatusChanged,
          OutboxUserBannedProcessor.BatchSize,
        )
        .andThen((outboxes) =>
  ...
  deleteSession(outbox: Outbox<OutboxUserBannedData>): ResultAsync<void> {
